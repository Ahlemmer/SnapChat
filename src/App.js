import { onAuthStateChanged } from 'firebase/auth';
import React, { useEffect } from 'react';
import { useDispatch, useSelector } from 'react-redux';
import { BrowserRouter,Route,Routes } from 'react-router-dom';
import './App.css';
import Chat from './Composent/Chat';
import ChatView from './Composent/ChatView';
import { auth } from './Composent/Firebase';
import Login from './Composent/Login';
import Preview from './Composent/Preview';
import WebcamCaPture from './Composent/WebcamCaPture';
import { login, Logout, SelactUser } from './features/App-Slice';


function App() {
  const user=useSelector( SelactUser)
  const dispatch=useDispatch()
  useEffect(() => {
    onAuthStateChanged(auth,(authUser)=>{
      if(authUser){
        dispatch(login({
               
          name:authUser.user.displayName,
          profilpic:authUser.user.photoURL,
          id:authUser.user.uid
        

      }))
      }
      else{
        dispatch(Logout())
      }
    })
  
  }, []);
  return (
    
    <div className="App">
      <BrowserRouter>
      {!user ? <Login/> :
      (  
        <>
        <img className='logo'src="data:image/png;base64," alt="" />
        
        <div className='app_body'>
        <div className='app_bodyBackground'>
      <Routes>
           <Route path='/Preview' element={<Preview/>} />
           <Route path='/Chats' element={<Chat/>}/>
           <Route path='/Chat/View' element={<ChatView/>}/>
           <Route path='/' element={<WebcamCaPture/>} index/>
      </Routes>
      </div>
     
    </div>
        </>
   )}
    
      </BrowserRouter>
     
    
    </div>
   
  );
}

export default App;
